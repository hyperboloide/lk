...
Below is an example of code that generates a license from a private key and
a struct containing and end date and a user email that is marshalled to json.
...
// Here we use a struct that is marshalled to json, but ultimatly all you need
// is a []byte.
doc := struct {
...
Before your execute your program you want to check the user license, here is how
with key generated from the previous example:

```go
// A previously generated license b32 encoded. In real life you should read
// it from a file and validate it before doing anything else...
...
// Now you just have to check the end date and if it's before time.Now(), then you can continue!
...
Bellow is a sample function that generate a key pair, signs a license and
verify it.
